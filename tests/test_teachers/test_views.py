...
def test_get_assignments_teacher_1(api_client, teacher_1):
    ...
    check if all the retrived assignments
    belong to the teacher for which we have asked.
    """
    response = api_client.get(
        reverse('teachers_assignments'),
...
@pytest.mark.django_db()
def test_invalid_assignment(api_client, teacher_2):
    grade = 'A'
    response = api_client.patch(
        reverse('teachers_assignments'),
        data=json.dumps({
    ...
    assert error['non_field_errors'] == ['Assignment does not exists.']